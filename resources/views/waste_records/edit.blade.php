...
@section('title', 'Izmeni zapis evidencije otpada - EcoFlow')

@section('content')
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Izmeni zapis evidencije otpada</h1>
            <p class="mt-2 text-sm text-gray-600">Ažurirajte podatke o zapisu</p>
        </div>
        ...
        <!-- Form -->
        <form action="{{ route('waste-records.update', $wasteRecord) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($companies as $company)
                            <option value="{{ $company->id }}" {{ old('company_id', $wasteRecord->company_id) == $company->id ? 'selected' : '' }}>
                                {{ $company->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($operators as $operator)
                            <option value="{{ $operator->id }}" {{ old('operator_id', $wasteRecord->operator_id) == $operator->id ? 'selected' : '' }}>
                                {{ $operator->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="waste_type" 
                        id="waste_type" 
                        value="{{ old('waste_type', $wasteRecord->waste_type) }}" 
                        required
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="kolicina" 
                        id="kolicina" 
                        value="{{ old('kolicina', $wasteRecord->kolicina) }}" 
                        step="0.01"
                        min="0"
                        required
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="kg" {{ old('jedinica_mere', $wasteRecord->jedinica_mere) == 'kg' ? 'selected' : '' }}>kg</option>
                        <option value="t" {{ old('jedinica_mere', $wasteRecord->jedinica_mere) == 't' ? 'selected' : '' }}>t</option>
                        <option value="l" {{ old('jedinica_mere', $wasteRecord->jedinica_mere) == 'l' ? 'selected' : '' }}>l</option>
                        <option value="m3" {{ old('jedinica_mere', $wasteRecord->jedinica_mere) == 'm3' ? 'selected' : '' }}>m³</option>
                    </select>
                    @error('jedinica_mere')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="nastao" {{ old('status', $wasteRecord->status) == 'nastao' ? 'selected' : '' }}>Nastao</option>
                        <option value="spreman_za_predaju" {{ old('status', $wasteRecord->status) == 'spreman_za_predaju' ? 'selected' : '' }}>Spreman za predaju</option>
                        <option value="predat" {{ old('status', $wasteRecord->status) == 'predat' ? 'selected' : '' }}>Predat</option>
                    </select>
                    @error('status')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="datum_nastanka" 
                        id="datum_nastanka" 
                        value="{{ old('datum_nastanka', $wasteRecord->datum_nastanka->format('Y-m-d')) }}" 
                        required
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="datum_predaje" 
                        id="datum_predaje" 
                        value="{{ old('datum_predaje', $wasteRecord->datum_predaje ? $wasteRecord->datum_predaje->format('Y-m-d') : '') }}"
                ...
                <div class="md:col-span-2">
                    ...
                    >{{ old('opis', $wasteRecord->opis) }}</textarea>
                    @error('opis')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
