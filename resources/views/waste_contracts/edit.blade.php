...
@section('title', 'Izmeni ugovor - EcoFlow')

@section('content')
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Izmeni ugovor</h1>
            <p class="mt-2 text-sm text-gray-600">Ažurirajte podatke o ugovoru</p>
        </div>
        ...
        <!-- Form -->
        <form action="{{ route('waste-contracts.update', $wasteContract) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($companies as $company)
                            <option value="{{ $company->id }}" {{ old('company_id', $wasteContract->company_id) == $company->id ? 'selected' : '' }}>
                                {{ $company->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($operators as $operator)
                            <option value="{{ $operator->id }}" {{ old('operator_id', $wasteContract->operator_id) == $operator->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="date_start" 
                        id="date_start" 
                        value="{{ old('date_start', $wasteContract->date_start->format('Y-m-d')) }}" 
                        required
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="date_end" 
                        id="date_end" 
                        value="{{ old('date_end', $wasteContract->date_end?->format('Y-m-d')) }}"
                        ...
                        @foreach($wasteTypes as $type)
                            <label class="flex items-center space-x-2 p-3 border border-gray-300 rounded-lg hover:bg-gray-50 cursor-pointer">
                                <input 
                                    ...
                                    name="waste_types[]" 
                                    value="{{ $type }}"
                                    {{ in_array($type, old('waste_types', $wasteContract->waste_types ?? [])) ? 'checked' : '' }}
                ...
                <div class="md:col-span-2">
                    ...
                    >{{ old('notes', $wasteContract->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
