...
@section('title', 'Izmeni lokaciju - EcoFlow')

@section('content')
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Izmeni lokaciju</h1>
            <p class="mt-2 text-sm text-gray-600">Ažurirajte podatke o lokaciji</p>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="magacin" {{ old('tip', $location->tip) == 'magacin' ? 'selected' : '' }}>Magacin</option>
                        <option value="prodavnica" {{ old('tip', $location->tip) == 'prodavnica' ? 'selected' : '' }}>Prodavnica</option>
                        <option value="pogon" {{ old('tip', $location->tip) == 'pogon' ? 'selected' : '' }}>Pogon</option>
                        <option value="kancelarija" {{ old('tip', $location->tip) == 'kancelarija' ? 'selected' : '' }}>Kancelarija</option>
                    </select>
                    @error('tip')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <input 
                        ...
                        name="adresa" 
                        id="adresa" 
                        value="{{ old('adresa', $location->adresa) }}" 
                        required
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="kontakt_osoba" 
                        id="kontakt_osoba" 
                        value="{{ old('kontakt_osoba', $location->kontakt_osoba) }}"
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="telefon" 
                        id="telefon" 
                        value="{{ old('telefon', $location->telefon) }}"
                ...
                <div>
                    ...
                    >
                        <option value="aktivna" {{ old('status', $location->status) == 'aktivna' ? 'selected' : '' }}>Aktivna</option>
                        <option value="neaktivna" {{ old('status', $location->status) == 'neaktivna' ? 'selected' : '' }}>Neaktivna</option>
                    </select>
                    @error('status')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
