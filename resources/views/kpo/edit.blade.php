...
@section('title', 'Izmeni KPO zapis - EcoFlow')

@section('content')
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Izmeni KPO zapis</h1>
            <p class="mt-2 text-sm text-gray-600">Ažurirajte podatke o KPO zapisu</p>
        </div>
        ...
        <!-- Form -->
        <form action="{{ route('kpo.update', $kpo) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($wastes as $waste)
                            <option value="{{ $waste->id }}" {{ old('waste_id', $kpo->waste_id) == $waste->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="datum" 
                        id="datum" 
                        value="{{ old('datum', $kpo->datum->format('Y-m-d')) }}" 
                        required
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="kolicina" 
                        id="kolicina" 
                        value="{{ old('kolicina', $kpo->kolicina) }}" 
                        required
                        step="0.01"
                        min="0.01"
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($nacini as $nacin)
                            <option value="{{ $nacin }}" {{ old('nacin_postupanja', $kpo->nacin_postupanja) == $nacin ? 'selected' : '' }}>
                                {{ $nacin }}
                            </option>
                        @endforeach
                    </select>
