...
@section('title', 'Izmeni evidencijoni list - EcoFlow')

@section('content')
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Izmeni evidencijoni list otpada</h1>
            <p class="mt-2 text-sm text-gray-600">Ažurirajte podatke o evidenciji</p>
        </div>
        ...
        <!-- Form -->
        <form action="{{ route('waste-evidence-sheets.update', $wasteEvidenceSheet) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($companies as $company)
                            <option value="{{ $company->id }}" {{ old('company_id', $wasteEvidenceSheet->company_id) == $company->id ? 'selected' : '' }}>
                                {{ $company->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="waste_type" 
                        id="waste_type" 
                        value="{{ old('waste_type', $wasteEvidenceSheet->waste_type) }}" 
                        required
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="godina" 
                        id="godina" 
                        value="{{ old('godina', $wasteEvidenceSheet->godina) }}" 
                        min="2000"
                        max="2100"
                        required
                ...
                <div>
                    ...
                    <input 
                        ...
                        id="ukupna_kolicina" 
                        value="{{ old('ukupna_kolicina', $wasteEvidenceSheet->ukupna_kolicina) }}" 
                        step="0.01"
                        min="0"
                        required
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="kg" {{ old('jedinica_mere', $wasteEvidenceSheet->jedinica_mere) == 'kg' ? 'selected' : '' }}>kg</option>
                        <option value="t" {{ old('jedinica_mere', $wasteEvidenceSheet->jedinica_mere) == 't' ? 'selected' : '' }}>t</option>
                        <option value="l" {{ old('jedinica_mere', $wasteEvidenceSheet->jedinica_mere) == 'l' ? 'selected' : '' }}>l</option>
                        <option value="m3" {{ old('jedinica_mere', $wasteEvidenceSheet->jedinica_mere) == 'm3' ? 'selected' : '' }}>m³</option>
                    </select>
                    @error('jedinica_mere')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    >{{ old('opis', $wasteEvidenceSheet->opis) }}</textarea>
                    @error('opis')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
