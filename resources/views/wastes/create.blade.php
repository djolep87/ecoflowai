...
@section('title', 'Dodaj otpad - EcoFlow')

@section('content')
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Dodaj novi otpad</h1>
        ...
        <!-- Form -->
        <form action="{{ route('wastes.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    >
                        <option value="">Prvo izaberite kompaniju</option>
                    </select>
                    @error('location_id')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="Papir" {{ old('tip_otpada') == 'Papir' ? 'selected' : '' }}>Papir</option>
                        <option value="Plastika" {{ old('tip_otpada') == 'Plastika' ? 'selected' : '' }}>Plastika</option>
                        <option value="Metal" {{ old('tip_otpada') == 'Metal' ? 'selected' : '' }}>Metal</option>
                        <option value="Elektronski otpad" {{ old('tip_otpada') == 'Elektronski otpad' ? 'selected' : '' }}>Elektronski otpad</option>
                        <option value="Staklo" {{ old('tip_otpada') == 'Staklo' ? 'selected' : '' }}>Staklo</option>
                        <option value="Bio otpad" {{ old('tip_otpada') == 'Bio otpad' ? 'selected' : '' }}>Bio otpad</option>
                        <option value="Ostalo" {{ old('tip_otpada') == 'Ostalo' ? 'selected' : '' }}>Ostalo</option>
                    </select>
                    @error('tip_otpada')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="datum_nastanka" 
                        id="datum_nastanka" 
                        value="{{ old('datum_nastanka', date('Y-m-d')) }}" 
                        required
                ...
                <div>
                    ...
                    <div class="px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl text-gray-700">
                        ...
                        <p class="text-xs text-gray-500 mt-2">Status se automatski postavlja na "Prijavljen" i može se menjati samo preko modula za preuzimanje.</p>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const allLocations = @json($locations->groupBy('company_id'));
            ...
            // If company is pre-selected (from old input), trigger change
            if (companySelect.value) {
                companySelect.dispatchEvent(new Event('change'));
                // Set old location value if exists
